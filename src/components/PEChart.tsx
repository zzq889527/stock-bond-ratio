...
export function PEChart({ data, indexId = 'hs300', isLandscape = false }: ValuationChartProps) {
  ...
  useEffect(() => {
    ...
    const meanVal = peValues.reduce((a, b) => a + b, 0) / peValues.length;
    const stdVal = Math.sqrt(peValues.reduce((sq, v) => sq + (v - meanVal) ** 2, 0) / peValues.length);
    const currentVal = peValues[peValues.length - 1];
    const percentile = (peValues.filter(v => v <= currentVal).length / peValues.length * 100).toFixed(0);

    const meanLine = peValues.map(() => meanVal);
    const sigmaUpper = peValues.map(() => meanVal + stdVal);
    const sigmaLower = peValues.map(() => meanVal - stdVal);

    const minVal = Math.min(...peValues);
    const maxVal = Math.max(...peValues);
    ...
    const option: echarts.EChartsOption = {
      ...
      color: ['#06b6d4', config.color, '#6b7280', '#84cc16', '#f97316'],
      animation: true,
      ...
      series: [
        ...
        },
        {
          ...
          label: {
            ...
            fontWeight: 'bold',
            formatter: `${currentVal.toFixed(1)}x · ${percentile}分位`
          },
          animationDuration: 1500,
          animationDelay: 1000
        }
